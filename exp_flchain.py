...
kf = KFold(n_splits=5)
data = flchain.read_df()
data = data.sample(frac=1).reset_index(drop=True)

grp = pd.get_dummies(data['flc.grp'],prefix='grp')

data = pd.concat([grp,data.iloc[:,[0,1,2,3,4,6,7,8,9]]],axis=1)
...
for train_index, test_index in kf.split(data):
    ...
    sorted_train = sorted(np.unique(df_train['futime']))
    N = len(df_train)
    id_list = []
    start_list = []
    end_list = []
    ...
    df_train = df_train.drop(columns=['futime','death'])

    # expand the validation data frame
    sorted_val = sorted(np.unique(df_val['futime']))
    N = len(df_val)
    id_list = []
    start_list = []
    end_list = []
    ...
    df_val = df_val.drop(columns=['futime','death'])

    # expand the test data frame by training partition
    durations_test = np.array(df_test['futime'])
    events_test = np.array(df_test['death'])
    ...
    df_test_1 = df_test_1.drop(columns=['futime','death'])
    ...
    df_test_2 = df_test_2.drop(columns=['futime','death'])
    ...
    for nn_config in nn_config_list:
        ...
        x_mean_1_tmp, x_std_1_tmp, loss_1 = model_1_tmp.train(
            x_train = df_train.groupby('id').apply(lambda x: x.values[:,np.r_[0:16,19,20]]),
            fail_train = df_train.groupby('id').apply(lambda x: x.values[:,18]),
            x_valid = df_val.groupby('id').apply(lambda x: x.values[:,np.r_[0:16,19,20]]),
            fail_valid = df_val.groupby('id').apply(lambda x: x.values[:,18])
            )

        # model 2
        model_2_tmp = hazardNN(nn_config)
        x_mean_2_tmp, x_std_2_tmp, loss_2 = model_2_tmp.train(
            x_train = df_val.groupby('id').apply(lambda x: x.values[:,np.r_[0:16,19,20]]),
            fail_train = df_val.groupby('id').apply(lambda x: x.values[:,18]),
            x_valid = df_train.groupby('id').apply(lambda x: x.values[:,np.r_[0:16,19,20]]),
            fail_valid = df_train.groupby('id').apply(lambda x: x.values[:,18])
            )
    ...
    x_test_1 = df_test_1.groupby('id').apply(lambda x: x.values[:,np.r_[0:16,18,19]])
    ...
    x_test_2 = df_test_2.groupby('id').apply(lambda x: x.values[:,np.r_[0:16,18,19]])
